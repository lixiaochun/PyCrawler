...
# UGirls
def ugirls():
    from ugirls import ugirls
    ugirls_path = os.path.join(ROOT_PATH, "ugirls")
    os.chdir(ugirls_path)
    ugirls.UGirls().main()
...
def weibo():
    ...
    os.chdir(weibo_path)
    for save_file in ["1931", "ATF", "idolschool", "lunar", "save_1", "save_2", "snh48"]:
        save_data_path = os.path.join(weibo_path, "info\\photo_%s.data" % save_file)
